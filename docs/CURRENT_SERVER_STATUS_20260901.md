...
- 活动 Git 仓库位于 `/wangbenyou-sulongjie/caimeng/qwen-codebase`，Python 工程文件直接位于仓库根目录，主网站位于 `online_page/`，Motion-LLM Research Atlas 位于 `online_page/motionllm-page/`。
- 规范数据位于 `/wangbenyou-sulongjie/caimeng/dataset`。
- Qwen 数据组织软链接共 10,524 个，断链 0。
- 最新网页包含 Codebase 主入口以及原有 3 个 HTML 页面；原页面检查 103 个本地目标，失败 0。
- 数据审计归档可以正常列出和解压。
